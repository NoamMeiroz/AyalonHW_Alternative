...
function getNearestWorkDay(date, hour) {
  ...
  if (now.getDay() >= 5) return getNearestWorkDay(now, hour);
  else {
...
function getFirstNumberInString(alphanum) {
  ...
  let numbers = null;
  if (!alphanum) return alphanum;

  if (!(typeof alphanum === "string" || alphanum instanceof String)) {
    if (isInteger(alphanum)) return alphanum;
    else return null;
  }
  firstChar = (alphanum.match(/[^0-9]/) || []).pop();
  if (!firstChar) {
    numbers = Number(alphanum);
    if (!isInteger(numbers)) numbers = null;
  } else {
    if (alphanum.search(firstChar) === 0) numbers = null;
    else {
...
function removeLastWordWithDigits(myString) {
  ...
  if (hasDigits(myString)) {
    ...
    if (hasDigits(building)) temp.pop();
  }
  return temp.join(" ");
}
