      ...
      (err, payload) => {
        ...
        for (i in payload) {
          ...
          if (employeesList[i].UPLOAD_ERROR === null) {
            ...
            for (direction of ROUTE_COLUMS) {
              for (solution of SOLUTIONS_LIST) {
                if (
                  employeesList[i][
                    `${direction}_${solution.toUpperCase()}_DISTANCE`
                  ] === null
                )
                  employeesList[i][
                    `${direction}_${solution.toUpperCase()}_DISTANCE`
                ...
                if (
                  employeesList[i][
                    `${direction}_${solution.toUpperCase()}_DURATION`
                  ] === null
                )
                  employeesList[i][
                    `${direction}_${solution.toUpperCase()}_DURATION`
          ...
          if (employeesList[i].BicycleDisqualifyReason) {
            ...
            employeesList[i].BICYCLE_DISQUALIFY_REASON =
          ...
          if (employeesList[i].WorkShuttleDisqualifyReason) {
            ...
            employeesList[i].WORK_SHUTTLE_DISQUALIFY_REASON =
              disqualifyReason.REASON;
          }
          ...
          if (employeesList[i].CarpoolDisqualifyReason) {
            ...
            employeesList[i].CARPOOL_DISQUALIFY_REASON =
              disqualifyReason.REASON;
          }
          ...
          if (employeesList[i].WalkingDisqualifyReason) {
            ...
            employeesList[i].WALKING_DISQUALIFY_REASON =
              disqualifyReason.REASON;
          }
...
const getCluster = (
  ...
  return new Promise(function (resolve, reject) {
    getEmployeesOfEmployer(
      ...
      .then((empList) => {
        if (empList.length === 0) return resolve(empList);
        let data = {
          ...
          .catch((error) => {
            ...
            if (res) {
              switch (res.data.code) {
                ...
                case 3001:
                  message =
                    "מספר העובדים המקסימלי לקלאסטר חייב להיות מספר חיובי ושלם";
                  break;
                case 3002:
                  message =
                    "גודל קבוצת הצימוד חייב להיות נמוך או שווה למספר העובדים";
                  break;
                case 3011:
