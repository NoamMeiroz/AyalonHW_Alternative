...
function employeesService(workerData) {
  return new Promise((resolve, reject) => {
    ...
    worker.on('exit', code => {
      if (code !== 0) return reject(new ServerError(500, `Worker stopped with exit code ${code}`));
    });
  });
}
...
async function run(employer, employees) {
  ...
  let state = employerSchema.STATE.READY; //await employeesService({ employer, employees })
  ...
  employerSchema.setEmploeeState(employer.id, state, (err, result) => {
    if (result) logger.debug(JSON.stringify(result));else {
  ...
  return new Promise(function (resolve, reject) {
    employeeSchema.getEmployeesOfEmployer(empId, [], [], (err, payload) => {
      ...
      for (i in payload) {
        ...
        employeesList[i].WORK_SITE = workSite.ADDRESS_STREET + " " + workSite.ADDRESS_BUILDING_NUMBER + ", " + workSite.ADDRESS_CITY;
