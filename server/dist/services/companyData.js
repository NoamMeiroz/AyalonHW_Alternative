...
function defined_structure(obj, attrs) {
  if (!obj) return {
    valid: false,
    attribute: "רשימת עובדים"
  };
  var tmp = obj;

  for (i = 0; i < attrs.length; ++i) {
    if (!tmp.hasOwnProperty(attrs[i])) return {
      valid: false,
      attribute: attrs[i]
    };
  }
...
saveEmployer = employer => {
  return new Promise(function (resolve, reject) {
    employerSchema.insertEmployer(employer, (err, data) => {
      if (!err) return resolve(data.dataValues);else {
...
const handleEmployerData = data => {
  return new Promise(function (resolve, reject) {
    ...
    if (!result.valid) {
      return reject(new ServerError(status = 400, message = `פרטי החברה לא כוללים נתוני ${result.attribute}`));
    } else {
      // get sectorlist
...
const readSheet = company_sheets => {
  return new Promise(function (resolve, reject) {
    ...
    if (employees == undefined) return reject(new ServerError(status = 400, message = "חוצץ פרטי עובדים חסר"));else if (employees[0] === undefined) return reject(new ServerError(status = 400, message = "חוצץ פרטי עובדים אינו מכיל מידע על העובדים")); // check if employees fields exists

    fields = Object.values(employeeFieldsName); //fields = fields.concat(Object.values(sitesFieldsName));
    ...
    Promise.all([handleEmployerData(employerData), siteData.handleSiteData(employees)]).then(result => {
      let employer = result[0];
      let siteList = result[1]; // save employer

      saveEmployer(employer).then(data => {
        let employer = data; // if employer is succeccfully saved the save siteList

        siteData.saveSites(employer.id, siteList).then(data => {
          employer.Sites = data;
