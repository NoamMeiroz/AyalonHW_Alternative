...
const insertBulk = (employees, callback) => {
  // Save  in the database
  Employee.bulkCreate(employees).then(data => {
    callback(null, data);
  }).catch(err => {
    callback(err, getMessage(err));
  });
...
const getEmployeesOfEmployer = (employerId, livingCity = [], workingCity = [], callback) => {
  ...
  if (livingCity && livingCity.length > 0) whereClause.where.CITY = {
    [Op.in]: livingCity
  };
  let include = [{
    model: EmployerSites,
  ...
  if (workingCity && workingCity.length > 0) include[0].where = {
    ADDRESS_CITY: {
      [Op.in]: workingCity
    }
  };
  ...
  Employee.findAll(whereClause).then(data => {
    callback(null, data);
  }).catch(err => {
    callback(err, getMessage(err));
  });
...
const getEmployees = (employerList, livingCity = [], workingCity = [], callback) => {
  ...
  if (employerList && employerList.length > 0) whereClause.where.employer_id = {
    [Op.in]: employerList
  };
  if (livingCity && livingCity.length > 0) whereClause.where.CITY = {
    [Op.in]: livingCity
...
const getPrecentFinished = (employerID, callback) => {
  ...
  isFinishedWithEmployees(employerID, (err, data) => {
    if (err) callback(err, getMessage(err)); // if result is true return 100 precent;
    else if (data) {
        return callback(null, 100);
          ...
          Promise.all([Employee.count({
            ...
            let precent = 0;
            let total = results[1];
            if (countFinished === 0) precent = 0;else {
