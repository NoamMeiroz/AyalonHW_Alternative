...
var server_app = express(); // synchronize with database
...
server_app.use("/api/reports", reportsRoutes); // error handling. must be last call 

server_app.use(function (err, req, res, next) {
  // set locals, only providing error in development
  if (err.message) {
    if (err.message instanceof ServerError) res.locals.message = err.message.message;else res.locals.message = err.message;
  } else res.locals.message = err;
  res.locals.error = req.app.get('env') === 'development' ? err : {}; // add this line to include winston logging

  if (err.message) logger.error(`${err.status || 500} - ${res.locals.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`); // render the error page
