...
class Companies extends Component {
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      this.props.longExecution &&
      this.props.longExecution.timestamp !== prevProps.longExecution.timestamp
    ) {
      ...
      let processID = setInterval(() => {
        this.props.checkProgress(employerID);
      }, 3000);
      // save process id
    ...
    if (
      this.props.loadData &&
      this.props.loadData.timestamp !== prevProps.loadData.timestamp
    ) {
...
export default requireAuth(connect(mapStateToProps, actions)(Companies));
