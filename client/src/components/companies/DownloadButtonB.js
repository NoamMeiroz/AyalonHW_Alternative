...
function DownloadButton({ csvData, fileName, employeesReady, uploadPrecent }) {
  const fileType =
    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8";
  const fileExtension = ".xlsx";
  const [uploadProgess, setUploadProgress] = React.useState(0);
  const [isEmployeesReady, setIsEmployeesReady] = React.useState(null);

  React.useEffect(() => {
    setUploadProgress(uploadPrecent);
    setIsEmployeesReady(employeesReady);
  }, [csvData, employeesReady, uploadPrecent]);
 ...
 */
const getTransitDescription = (transitDescription) => {
  if (transitDescription.error)
    return { description: transitDescription.error };
  let bestRoute = findFastestRoute(transitDescription);
  ...
  let result = {
    description: description,
    duration: bestRoute.legs[0].duration.value,
 ...
 */
const getBicycleDescription = (bicycleDescription) => {
  if (bicycleDescription.error)
    return { description: bicycleDescription.error };
  let bestRoute = findFastestRoute(bicycleDescription);
  const steps = bestRoute.legs[0].steps;
  let bicycle = [];
  let description = "";
  steps.forEach((step, index, array) => {
 ...
 */
const getWalkingDescription = (walkingDescription) => {
  ...
  let result = {
    description: description,
    duration: walkingDescription.legs[0].duration.value,
    distance: walkingDescription.legs[0].distance.value,
  };
  return result;
};
 ...
 * @param {json} bicycleDescription
 */
const getDrivingDescription = (drivingDescription) => {
  if (drivingDescription.error)
    return { description: drivingDescription.error };
  let bestRoute = findFastestRoute(drivingDescription);
  const steps = bestRoute.legs[0].steps;
  let driving = [];
  let description = "";
  steps.forEach((step, index, array) => {
...
const saveEmployeesList = (employerId, fileName) => {
  axios
    .get(`/api/employer/${employerId}/employee`, {
    ...
    .then((payload) => {
      ...
      if (employeeList && !(typeof employeeList === "string")) {
        employeeList.forEach((emp) => {
          try {
            ...
            try {
              ...
              if (
                emp.bicycling !== undefined &&
                emp.bicycling.includes("שגיאה")
              )
              ...
              if (
                emp.transit_home !== undefined &&
                emp.transit_home.includes("שגיאה")
              )
                emp.ERROR = emp.ERROR + "\nחישוב מסלול תחבורה ציבורית נכשל.";
              if (
                emp.bicycling_home !== undefined &&
                emp.bicycling_home.includes("שגיאה")
              )
                emp.ERROR = emp.ERROR + "\nחישוב מסלול רכיבה נכשל.";
              if (
                emp.walking_home !== undefined &&
                emp.walking_home.includes("שגיאה")
              )
                emp.ERROR = emp.ERROR + "\nחישוב מסלול הליכה נכשל.";
              if (
                emp.driving_home !== undefined &&
                emp.driving_home.includes("שגיאה")
              )
                emp.ERROR = emp.ERROR + "\nחישוב מסלול נהיגה עצמית נכשל.";
            } catch (error) {
              console.log(error);
        ...
        exportToCSV(employeeList, fileName);
      }
    })
    .catch((err) => {
  ...
  const exportToCSV = async (csvData, fileName) => {
    ...
    ws.columns = [
      ...
      {
        header: "שעת היציאה ממקום העבודה",
        key: "RETURN_HOUR_TO_HOME",
        width: 15,
      },
      {
        header: "ציון מקורי\nקיצור שעות העבודה",
        key: "SHORT_HOURS_GRADE",
        width: 15,
      },
      {
        header: "ציון מחושב\nקיצור שעות העבודה",
        key: "FINAL_SHORT_HOURS_GRADE",
        width: 15,
      },
      {
        header: "ציון מקורי\nהזזת זמן הגעה לעבודה",
        key: "SHIFTING_HOURS_GRADE",
        width: 15,
      },
      {
        header: "ציון מחושב\nהזזת זמן הגעה לעבודה",
        key: "FINAL_SHIFTING_HOURS_GRADE",
        width: 15,
      },
      { header: "ציון מקורי\nדו גלגלי-אופניים", key: "BICYCLE_GRADE", width: 15 },
      {
        header: "ציון מחושב\nדו גלגלי-אופניים",
        key: "FINAL_BICYCLE_GRADE",
        width: 15,
      },
      { header: "ציון מקורי\nדו גלגלי-קורקינט", key: "SCOOTER_GRADE", width: 15 },
      {
        header: "ציון מחושב\nדו גלגלי-קורקינט",
        key: "FINAL_SCOOTER_GRADE",
        width: 15,
      },
      {
        header: "ציון מקורי\nShuttle On Demand",
        key: "PERSONALIZED_SHUTTLE_GRADE",
        width: 15,
      },
      {
        header: "ציון מחושב\nShuttle On Demand",
        key: "FINAL_PERSONALIZED_SHUTTLE_GRADE",
        width: 15,
      },
      {
        header: "ציון מקורי\nשאטלים מטעם העבודה",
        key: "WORK_SHUTTLE_GRADE",
        width: 15,
      },
      {
        header: "ציון מחושב\nשאטלים מטעם העבודה",
        key: "FINAL_WORK_SHUTTLE_GRADE",
        width: 15,
      },
      {
        header: "ציון מקורי\nCarshare/Vanshare",
      ...
      {
        header: "ציון מחושב\nCarshare/Vanshare",
        key: "FINAL_CARSHARE_GRADE",
        width: 15,
      },
      { header: "ציון מקורי\nCarpool/Vanpool", key: "CARPOOL_GRADE", width: 15 },
      {
        header: "ציון מחושב\nCarpool/Vanpool",
        key: "FINAL_CARPOOL_GRADE",
        width: 15,
      },
      { header: "ציון מקורי\nמוניות שיתופיות", key: "CABSHARE_GRADE", width: 15 },
      {
        header: "ציון מחושב\nמוניות שיתופיות",
        key: "FINAL_CABSHARE_GRADE",
        width: 15,
      },
      {
        header: "ציון מקורי\nתחבורה ציבורית",
        key: "PUBLIC_TRANSPORT_GRADE",
        width: 15,
      },
      {
        header: "ציון מחושב\nתחבורה ציבורית",
        key: "FINAL_PUBLIC_TRANSPORT_GRADE",
        width: 15,
      },
      ...
      {
        header: "ציון מקורי\nעבודה מהבית",
        key: "WORKING_FROM_HOME_GRADE",
        width: 15,
      },
      {
        header: "ציון מחושב\nעבודה מהבית",
        key: "FINAL_WORKING_FROM_HOME_GRADE",
        width: 15,
      },
      {
        header: "ציון מקורי\nעבודה במרכזים שיתופיים",
        key: "SHARED_WORKSPACE_GRADE",
        width: 15,
      },
      {
        header: "ציון מחושב\nעבודה במרכזים שיתופיים",
        key: "FINAL_SHARED_WORKSPACE_GRADE",
        width: 15,
      },
      {
        header: "ציון מקורי\nשינוי ימי הגעה לעבודה",
        key: "SHIFTING_WORKING_DAYS_GRADE",
        width: 15,
      },
      {
        header: "ציון מחושב\nשינוי ימי הגעה לעבודה",
        key: "FINAL_SHIFTING_WORKING_DAYS_GRADE",
        width: 15,
      },
      ...
      {
        header: "אופניים למקום העבודה\nמשך בשניות",
        key: "bicycling_duration",
        width: 15,
      },
      {
        header: "אופניים למקום העבודה\nמרחק במטרים",
        key: "bicycling_distance",
        width: 15,
      },
      ...
      {
        header: "תחבורה ציבורית חזרה הביתה\nמשך בשניות",
        key: "transit_home_duration",
        width: 15,
      },
      {
        header: "תחבורה ציבורית חזרה הביתה\nמרחק במטרים",
        key: "transit_home_distance",
        width: 15,
      },
      { header: "אופניים חזרה הביתה\nמסלול", key: "bicycling_home", width: 50 },
      {
        header: "אופניים חזרה הביתה\nמשך בשניות",
        key: "bicycling_home_duration",
        width: 15,
      },
      {
        header: "אופניים חזרה הביתה\nמרחק במטרים",
        key: "bicycling_home_distance",
        width: 15,
      },
      { header: "הליכה חזרה הביתה\nמסלול", key: "walking_home", width: 50 },
      {
        header: "הליכה חזרה הביתה\nמשך בשניות",
        key: "walking_home_duration",
        width: 15,
      },
      {
        header: "הליכה חזרה הביתה\nמרחק במטרים",
        key: "walking_home_distance",
        width: 15,
      },
      { header: "נהיגה חזרה הביתה\nמסלול", key: "driving_home", width: 50 },
      {
        header: "נהיגה חזרה הביתה\nמשך בשניות",
        key: "driving_home_duration",
        width: 15,
      },
      {
        header: "נהיגה חזרה הביתה\nמרחק במטרים",
        key: "driving_home_distance",
        width: 15,
      },
    ];
  
    ws.getColumn("ERROR").alignment = {
      vertical: "top",
      horizontal: "right",
      wrapText: true,
      readingOrder: "rtl",
          ...
          rules: [
            {
              type: "expression",
              formulae: [
                `IF($${orig_mark}$${rowNumber}<>$${final_mark}${rowNumber}, IF(ISBLANK($${final_mark}${rowNumber}),0, 1), 0)`,
              ],
              style: {
                fill: {
                  type: "pattern",
                  pattern: "solid",
                  bgColor: { argb: "D0bbe33d" },
                },
              },
            },
          ],
  ...
  if (isEmployeesReady === 1)
      ...
      <div>
        <Tooltip title="דוח טעינה">
          <IconButton
            ...
            style={{ padding: "2px" }}
            onClick={(e) => {
              saveEmployeesList(csvData.id, fileName);
            }}
          >
            <SaveRoundedIcon />
          </IconButton>
    ...
    jsx = (
      <Grid container>
        ...
        >
          <CircularProgress variant="determinate" value={uploadProgess} />
          <Box
            top={0}
            left={0}
            bottom={0}
